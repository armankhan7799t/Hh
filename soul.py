...
bot = telebot.TeleBot('7854080871:AAHy46Ktk1TLk5l4FRO32e704izh3Nwi_h4')
# DEVELOPER --> @S4_LUCHI
# Admin user IDs
admin_id = ["","7933339379"]
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "PAPA SE BAAT KAR = @FATHER_OF_HAX."
...
def start_attack_reply(message, target, port, time):
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("SUPPORT", url="https://t.me/FATHER_OF_HAX")
    )
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "💠𝐈𝐭'𝐬 𝐓𝐢𝐦𝐞 𝐓𝐨 𝐀𝐭𝐭𝐚𝐜𝐤💠 \n\n/arman <𝐭𝐚𝐫𝐠𝐞𝐭> <𝐩𝐨𝐫𝐭> <𝐭𝐢𝐦𝐞>\n\n READY FOR ATTACK"  # Updated command syntax
    else:
        response = "𝐔𝐧𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐓𝐨 𝐔𝐬𝐞 𝐏𝐥𝐞𝐚𝐬𝐞 𝐃𝐌 𝐭𝐨 @FATHER_OF_HAX"
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/FATHER_OF_HAX"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['arman'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> arman \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑  \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 ARMAN TEAM.
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/FATHER_OF_HAX'),
        telebot.types.InlineKeyboardButton('Support', url='https://t.me/FATHER_OF_HAX')
    )
...
def welcome_start(message):
    ...
    response += '/arman :--> 𝐋𝐚𝐮𝐧𝐜𝐡 𝐚𝐧 𝐚𝐭𝐭𝐚𝐜𝐤\n'
